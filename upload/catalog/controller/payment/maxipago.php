...
class ControllerPaymentMaxipago extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->load->view('payment/maxipago.tpl', $data);
    }
    ...
    public function synchronize()
    {
        try
        {
            ...
            $this->loadSynchronizeDependencies();
    ...
    private function loadSynchronizeDependencies()
    {
        $this->load->model('payment/maxipago');
    }
    ...
    private function getSyncFromDate()
    {
        try
        {
            ...
            return $date->format('Y-m-d 00:00:00');$cancelledStatus = $this->config->get('maxipago_order_cancelled');
    ...
    public function synchronize_bkp()
    {
        $this->validateRequestKey();
        ...
        $sql = 'SELECT * FROM `' . DB_PREFIX . 'maxipago_transactions` ' . 
            ...
            'AND `response_message` in ("ISSUED", "VIEWED", "BOLETO ISSUED", "BOLETO VIEWED", "PENDING", "PENDING CONFIRMATION", "AUTHORIZED", "ENROLLED", "IN PROGRESS", "UNKNOW RESPONSE")';
