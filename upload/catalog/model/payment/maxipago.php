...
class ModelPaymentMaxipago extends Model
{
    ...
    public function ticketMethod($order_info)
    {
        ...
        if ($methodEnabled) {
            ...
            $data = array(
                ...
                'bname' => $billingAddress['firstname'] . ' ' . $billingAddress['lastname'],
    ...
    public function recurringMethod($order_data, $recurring_data)
    {
        ...
        $has_trial = $recurring_data['recurring']['trial'] == "1";
        $trial_is_paid = $has_trial ? ((float) $recurring_data['recurring']['trial_price']) > 0 : false;
        ...
        if($has_trial)
        {
            ...
            $frequency = $this->getFrequencyFromCycle($recurring_data['recurring']['cycle'], $recurring_data['recurring']['frequency']);
            $request_data['frequency'] = $frequency;

            $period = $this->getPeriodFromFrequency($recurring_data['recurring']['frequency']);
            $request_data['period'] = $period;

            $trial_frequency = $this->getFrequencyFromCycle($recurring_data['recurring']['trial_cycle'], $recurring_data['recurring']['trial_frequency']);
            $trial_period = $this->getPeriodFromFrequency($recurring_data['recurring']['trial_frequency']);
            ...
            $installments = $this->getRecurringInstallments($recurring_data['recurring']['duration'], $period, $frequency);
    ...
    protected function _getAddress($order_info)
    {
        ...
        return array(
            'billing' => $billing,
            'shipping' => $this->cart->hasShipping() ? $shipping : $billing,
            'telephone' => $telephone
        );
    }
