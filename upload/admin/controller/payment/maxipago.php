...
class ControllerPaymentMaxipago extends Controller
{
    ...
    public function install()
    {
        try {
            ...
            if (version_compare(VERSION, '2.2', '>=')) {
                //admin/model/payment/maxipago.php
                $this->model_extension_event->addEvent('maxipago_delete', 'admin/model/sale/order/deleteOrder/before', 'payment/maxipago/reverse');
                //catalog/controller/payment/maxipago.php
                $this->model_extension_event->addEvent('maxipago_change', 'catalog/controller/api/order/history/after', 'payment/maxipago/change');
            } else {
                //admin/model/payment/maxipago.php
                $this->model_extension_event->addEvent('maxipago_delete', 'pre.order.delete', 'payment/maxipago/reverse');
                //admin/model/payment/maxipago.php
                $this->model_extension_event->addEvent('maxipago_change', 'post.order.history.add', 'payment/maxipago/change');
            }
        } catch (Exception $e) {
    ...
    {
        $this->load->model('extension/event');
        $this->model_extension_event->deleteEvent('maxipago_delete');
        $this->model_extension_event->deleteEvent('maxipago_change');
    }
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->_validate()) {
            ...
            $this->response->redirect($this->url->link('payment/maxipago', 'token=' . $this->session->data['token'], true));
        }
        ...
        $data['sync_url'] = $this->url->link('payment/maxipago/syncronize', $params, true);
    ...
    private function getCatalogUrl($function_name)
    {
        ...
        $base_url = new Url($use_ssl);
        $base_url->addRewrite($this);
        $store_key = $this->config->get('maxipago_consumer_key');
        return $base_url->link('payment/maxipago/' . $function_name, '&mpKey=' . $store_key, $use_ssl);
    }
    ...
    private function loadSyncronizeDependencies()
    {
        try
        {
            $this->load->language('payment/maxipago');
    ...
    private function validateRequestConsumerKey()
    {
        try
        {
    ...
    private function getSyncronizableTransactions()
    {
        try
        {
    ...
    private function orderIsFinished($transaction, $order)
    {
        $approvedStatus = $this->config->get('maxipago_order_approved');
        $cancelledStatus = $this->config->get('maxipago_order_cancelled');
        $refundedStatus = $this->config->get('maxipago_order_refunded');
        ...
        $finishedStatus = array(
            $approvedStatus, $cancelledStatus, $refundedStatus
        );

        $finishedResponses = array(
            'CAPTURED', 'PAID', 'CANCELED', 'VOIDED', 'REFUNDED'
        );
     ...
     */
    public function syncronize_bkp()
    {
        $success = array();
        $ordersUpdated = array();
    ...
    private function synchronizeCanceledOrders()
    {
        ...
        foreach($possibleCancelableOrders as $possibleCancelableOrder)
        {
            ...
            if($transactionQuery->num_rows > 0)
            {
                ...
                if($this->model_payment_maxipago->reverse($possibleCancelableOrder))
                {
    ...
    protected function synchronizeRefundedOrders()
    {
        ...
        $sql = 'SELECT * FROM `' . DB_PREFIX . 'order`' .
            'WHERE `date_added` > "' . $minimum_creation_date . '" ' .
            'AND `payment_code` = "maxipago" ' .
            'AND `order_status_id` in (' . $refund_order_status_id;

        $refund_on_cancel = $this->config->get('maxipago_cc_order_reverse');

        if($refund_on_cancel)
        {
            $cancel_order_status_id = $this->config->get('maxipago_order_cancelled');
            $sql .= ', ' . $cancel_order_status_id . ')';
        } else
        {
            $sql .= ')';
        }
